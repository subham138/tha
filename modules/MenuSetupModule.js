...
const BreakfastSave =
  ...
  (req, res, next) => {
    ...
    res.json({
      fileUrl: "http://localhost:3000/assets/files/" + req.file.filename,
    });
  });
...
const MenuImageSave = async (data, cov_img_path, top_img_path) => {
  ...
  var chk_dt = await Check_Data(
    (db_name = "td_other_image"),
    (whr = `WHERE hotel_id = "${data.hotel_id}" AND srv_res_flag = "${data.srv_res_flag}" AND srv_res_id = "${data.srv_res_id}" AND menu_id = "${data.menu_id}"`)
  );
  if (chk_dt > 1) {
...
const OtherImageSave = async (data, menu_img) => {
  ...
  if (Array.isArray(menu_img)) {
    for (let i = 0; i < menu_img.length; i++) {
      let menu_data = await F_Select(
        `SELECT * FROM td_menu_image WHERE hotel_id = "${data.hotel_id}" AND srv_res_flag = "${data.srv_res_flag}" AND srv_res_id = "${data.srv_res_id}" AND menu_id = "${data.menu_id}" AND menu_img = "${menu_img[i].filename}"`
      );
    ...
    if (menu_img) {
      let menu_data = await F_Select(
        `SELECT * FROM td_menu_image WHERE hotel_id = "${data.hotel_id}" AND srv_res_flag = "${data.srv_res_flag}" AND srv_res_id = "${data.srv_res_id}" AND menu_id = "${data.menu_id}" AND menu_img = "${menu_img}"`
      );
      if (menu_data.msg.length < 1) {
        sql = `INSERT INTO td_menu_image (hotel_id, srv_res_flag, srv_res_id, menu_id, active_flag, menu_url, menu_img, created_by, created_dt) VALUES
...
const SectionImageSave = async (data, sec_img) => {
  ...
  if (Array.isArray(sec_img)) {
    for (let i = 0; i < sec_img.length; i++) {
      let menu_data = await F_Select(
        `SELECT * FROM td_section_image_request WHERE restaurant_id = "${data.restaurant_id}" AND menu_id = "${data.menu_id}" AND sec_img = "${sec_img[i].filename}"`
      );
      if (menu_data.msg.length < 1) {
    ...
    if (sec_img) {
      let menu_data = await F_Select(
        `SELECT * FROM td_section_image_request WHERE restaurant_id = "${data.restaurant_id}" AND menu_id = "${data.menu_id}" AND sec_img = "${sec_img}"`
      );
      if (menu_data.msg.length < 1) {
        sql = `INSERT INTO td_section_image_request (restaurant_id, menu_id, active_flag, sec_img, sec_url, created_by, created_dt) VALUES
  ...
  var chk_dt = await Check_Data(
    (db_name = "td_section_image_request"),
    (whr = `WHERE restaurant_id = "${data.restaurant_id}" AND menu_id = "${data.menu_id}"`)
  );
  if (chk_dt > 1) {
...
const MonthDateSave = async (data) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    data.month_day.forEach(async (d) => {
      if (d.dt > 0) {
        var chk_dt = await Check_Data(
          (db_name = "td_date_time"),
          (whr = `WHERE hotel_id = "${data.hotel_id}" AND srv_res_flag = "${data.srv_res_flag}" AND srv_res_id = "${data.srv_res_id}" AND menu_id = "${data.menu_id}" AND month_day = "${d.dt}"`)
        );
        if (chk_dt > 1) {
...
const ItemSave = (data) => {
  ...
  if (data.id > 0) {
    ...
    sql = `INSERT INTO md_service_items (hotel_id, section_id, item_name, created_by, created_dt)
     VALUES ("${data.hotel_id}", "${data.sec}", "${data.item_name}", "${data.hotel_id}", "${datetime}")`;
  }
  return new Promise((resolve, reject) => {
    db.query(sql, (err, lastId) => {
      if (err) {
...
const GenerateQr = async (data) => {
  const qrCode = await create(data.url, data.img, 150, 50, data);
  let chk_dt = await Check_Data(
    (db_name = "md_url_custom"),
    (whr = `WHERE restaurant_id = "${data.res_id}" AND venue_id = "${data.venue_id}"`)
  );
  var sql = "";
  if (chk_dt > 1) {
